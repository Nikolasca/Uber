...
public class UsuarioController{
    ...
    @ResponseBody
    public String  Register(@RequestParam("name") String name, @RequestParam("email") String email, @RequestParam("pass") String pass, @RequestParam("type") String type) {
        String mensaje = "";
    ...
    public String Login(@RequestParam("user") String user, @RequestParam("pass") String pass){
        ...
        for (Usuario usuario : usuarios) {
            if ((usuario.getUsuario().compareTo(user) == 0) && (usuario.getPassword().compareTo(pass) == 0)) {
                x = true;
                return mensaje = "Usuario Aceptado,"+usuario.getUsuario()+","+usuario.getPassword()+","+usuario.getTipo_Usuario()+","+usuario.getEmail()+",";
            }
